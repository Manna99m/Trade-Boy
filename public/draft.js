...
document.getElementById('btn-random').addEventListener('click', () => {
    ...
    socket.emit('joinRandom', user);
});

document.getElementById('btn-create').addEventListener('click', () => {
    lobbyStatus.textContent = "Creating room...";
    socket.emit('createCustom', user);
});
...
document.getElementById('btn-join').addEventListener('click', () => {
    ...
    if (roomId) {
        ...
        socket.emit('joinCustom', { roomId, username: user });
        myRoomId = roomId;
    }
});
...
function renderChoice() {
    ...
    document.getElementById('draft-title').innerHTML = `Choose a <span style="color: #3b82f6">${type.toUpperCase()}</span>!`;
...
window.selectCard = function(index, isAuto = false) {
    ...
    const rejected = choicePair[index === 0 ? 1 : 0];
    
    socket.emit('makeChoice', { roomId: myRoomId, index: choiceIndex, picked, rejected });
    
    const path = currentTypes[choiceIndex] === 'hero' ? 'heroes/' : 'pets/';
    const myPickSlot = Array.from(document.querySelectorAll('#my-progress .draft-slot')).slice(0, 3).find(s => s.innerHTML === '?');
    const oppRejectSlot = Array.from(document.querySelectorAll('#opponent-progress .draft-slot')).slice(3, 6).find(s => s.innerHTML === '?');
    
    const selectedImg = document.getElementById(`img-${index}`);
    const rejectedImg = document.getElementById(`img-${index === 0 ? 1 : 0}`);
    
    if (myPickSlot && selectedImg && oppRejectSlot && rejectedImg && !isAuto) {
        animateToSlot(selectedImg, myPickSlot, path, picked, true);
        animateToSlot(rejectedImg, oppRejectSlot, path, rejected, false);
    } else {
        if (myPickSlot) {
        ...
        if (oppRejectSlot) {
            oppRejectSlot.innerHTML = `<img src="${path}${rejected}">`;
            oppRejectSlot.classList.add('filled');
        }
    }
...
socket.on('opponentSelected', () => {
    const oppPickSlot = Array.from(document.querySelectorAll('#opponent-progress .draft-slot')).slice(0, 3).find(s => s.innerHTML === '?' && !s.classList.contains('green'));
    if (oppPickSlot) {
        oppPickSlot.classList.add('green');
    }
    
    const myRejectSlot = Array.from(document.querySelectorAll('#my-progress .draft-slot')).slice(3, 6).find(s => s.innerHTML === '?' && !s.classList.contains('green'));
    if (myRejectSlot) {
        myRejectSlot.classList.add('green');
    }
});
